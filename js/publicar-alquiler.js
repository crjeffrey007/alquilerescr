...
document.getElementById('form-alquiler')?.addEventListener('submit', async function(e){
  ...
  const doc = {
    ...
    emailUsuario: (auth.currentUser && auth.currentUser.email) || data.email || ''
  };
  await db.collection('alquileres').add(doc);

  // Notificaciones: obtengo admins desde Firestore y se lo paso a Web3Forms
  ...
  showMsg(document.getElementById('msg'), 'Enviado. Pendiente de aprobación.');
  form.reset();
});
